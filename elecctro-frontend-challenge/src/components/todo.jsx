...
function Todo({ todo, setTodos, todos }) {
  ...
  const completeTodo = () => {
    axios.patch(`/todo/${todo.taskId}`, {
      ...
      Swal.fire('Sucesso', 'Tarefa terminada', 'success');
    ...
    <div className={todo.completedAt ? 'todo todo-completed' : 'todo'}>
      ...
      <div className="todo-buttons">
        {!todo.completedAt && !editing && (
          <div>
            <button type="button" onClick={completeTodo} className="complete-button">
              <svg style={{ width: '24px', height: '24px' }} viewBox="0 0 24 24">
                <path fill="currentColor" d="M9,20.42L2.79,14.21L5.62,11.38L9,14.77L18.88,4.88L21.71,7.71L9,20.42Z" />
              </svg>
            </button>
            <button type="button" onClick={() => setEditing(true)} className="edit-button">
              <svg style={{ width: '24px', height: '24px' }} viewBox="0 0 24 24">
        ...
        {!editing && (
          <button type="button" onClick={deleteTodo} className={todo.completedAt ? 'delete-button-rounded' : 'delete-button'}>
            <svg style={{ width: '24px', height: '24px' }} viewBox="0 0 24 24">
